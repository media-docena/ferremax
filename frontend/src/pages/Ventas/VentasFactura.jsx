...
function VentasFactura() {
  ...
  const handleDescargarPDF = async () => {
    try {
      ...
      try {
        ...
        logger.error('Error al obtener el logo SVG');
      }
  ...
  if (status === 404 || !venta) {
    return (
      <p className='text-red-600'>{message || 'No se encontró la venta'}</p>
    );
  }
      ...
      <div className='flex justify-center gap-6'>
        <div>
          <div
            id='factura-export'
            ref={facturaRef}
            className='w-175 bg-white p-6 rounded-lg shadow-lg'
          >
            ...
            <div className='flex mt-5 text-lg'>
              ...
              <div className='mt-5'>
                ...
                <p className='mt-5'>
                  ${Number(venta.totalVenta).toLocaleString('es-AR')}
                </p>
              </div>
            </div>
          </div>
