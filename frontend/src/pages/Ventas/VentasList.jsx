          ...
          <div className='bg-white p-6 rounded-lg shadow-lg'> {/* Contenedor Lista de Productos */}
            <div className='justify-between items-center mb-6 flex-wrap gap-4'>
              <div className='flex justify-between items-center gap-4'>
                ...
                <div>
                  <SearchBar placeholder="Buscar producto..." onSearch={handleSearch}/>
                </div>
              </div>
                  ...
                  <tbody className='divide-y divide-gray-200 text-sm'>
                    {products.map((product) => {
                      ...
                      <tr key={product.idProducto}>
                        ...
                        <td className='p-4'>{product.categoria?.nombre || 'Sin categoria'}</td>
                        <td className='p-4'>${Number(product.precio).toLocaleString('es-AR')}</td>
                      ...
                      {productsInCart.map((product) => (
                        <tr>
                          <td className='p-2'>
                            <div>
                              <p>{product.nombre}</p>
                              <p className='text-gray-600'>${Number(product.precio).toLocaleString('es-AR')}</p>
                            </div>
                          </td>
                          <td className='p-2'>
                            <div className='justify-center items-center flex gap-2 wmax-w-[30px]'>
                              <ActionIconButton icon={<RemoveIcon/>} type="button" onClick={() => decrementarCantidad(product.idProducto)}/>
                              <input type="text" className='w-10 text-center' defaultValue='1' value={product.cantidad} onChange={(e) => actualizarCantidad(product.idProducto, parseInt(e.target.value) || 1)}/>
                              <ActionIconButton icon={<AddIcon/>} type="button" onClick={() => incrementarCantidad(product.idProducto)}/>
                            </div>
                          </td>
                          <td>
                            <div className='p-2'>${Number(product.subtotal).toLocaleString('es-AR')}</div>
                          </td>
                          <td>
                            <ActionIconButton icon={<DeleteIcon/>} type="button" onClick={() => eliminarProducto(product.idProducto)}/>
                          </td>
                        </tr>
                      ))}
                    </tbody>
